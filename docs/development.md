# Development manual for Pyraclaw
This guide will show you how to setup a local development environment for Pyraclaw in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Pyraclaw runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- Pyraclaw is available on GitHub [github.com/pyraclaw/pyraclaw](https://github.com/pyraclaw/pyraclaw).
- You can download the files using a browser and extract or run `git clone https://github.com/pyraclaw/pyraclaw` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/pyraclaw/pyraclaw`, so my project folder is `~/Desktop/pyraclaw`.
...
- Pyraclaw comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/pyraclaw`.
...
## Step 4: Run Pyraclaw in the IDE
Great work! Now you should be able to run Pyraclaw from your IDE including real-time debugging.
...
After inserting my API key in settings, my Pyraclaw instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Pyraclaw in Docker
...
1. Pull the docker image `pyraclaw/pyraclaw` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a0` folder mapped to `/Users/frdel/Desktop/pyraclaw`:
...
You have successfully set up a complete Pyraclaw development environment! You now have:
...
- ✅ The ability to develop, debug, and test Pyraclaw features seamlessly

You're now ready to contribute to Pyraclaw, create custom extensions, or modify the framework to suit your needs. Happy coding! 💻✨
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t pyraclaw-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
